...
func (ct *categoryAPI) UpdateCategory(c *gin.Context) {
	idParam := c.Param("id")
	categoryID, parseErr := strconv.Atoi(idParam)
	if parseErr != nil {
	...
	if bindErr := c.ShouldBindJSON(&category); bindErr != nil {
		c.JSON(http.StatusBadRequest, model.ErrorResponse{Error: bindErr.Error()})
		return
	}

	if updateErr := ct.categoryService.Update(categoryID, category); updateErr != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: updateErr.Error()})
		return
	}
...
func (ct *categoryAPI) DeleteCategory(c *gin.Context) {
	...
	if deleteErr := ct.categoryService.Delete(categoryID); deleteErr != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: deleteErr.Error()})
		return
	}
...
func (ct *categoryAPI) GetCategoryList(c *gin.Context) {
	if categories, fetchErr := ct.categoryService.GetList(); fetchErr != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: fetchErr.Error()})
		return
	} else {
