...
func LoanReport(data LoanData) LoanRecord {
	if len(data.Data) != 0{
		...
		loadRecord.MonthDate = fmt.Sprintf("%s %s", date[1], date[2])

		loadRecord.EndBalance = endBalance
		loadRecord.Borrowers = borrowers
...
func RecordJSON(record LoanRecord, path string) error {
	...
	jsonData, errMarshal := json.Marshal(record)
	if errMarshal != nil {
		return errMarshal
	}
	errWriteFile := os.WriteFile(path, jsonData, 0644)
	if errWriteFile != nil {
		return errWriteFile
	}
